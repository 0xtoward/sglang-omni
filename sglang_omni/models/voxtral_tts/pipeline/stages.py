...
def create_vocoder_executor(
    ...
    *,
    ...
    ``max_batch_size`` defaults to 16 — the generation stage's
    ``max_running_requests`` cap and the measured throughput knee (a controlled
    back-to-back sweep showed req/s rising monotonically to batch 16, ~+6% over
    serial, the decoder being compute-bound past that); a longer
    ``max_batch_wait_ms`` only adds latency under load, so it stays at 2.
    """
    checkpoint_dir = _resolve_checkpoint(model_path)
    if gpu_id is not None:
        device = f"cuda:{gpu_id}"
    # Optional override of the vocoder coalescing batch size (1 disables batching)
    # and the coalescing wait window (longer fills larger batches under load).
    max_batch_size = int(
        os.environ.get("SGLANG_OMNI_VOXTRAL_VOCODER_MAX_BATCH", max_batch_size)
    )
    max_batch_wait_ms = int(
        os.environ.get("SGLANG_OMNI_VOXTRAL_VOCODER_MAX_WAIT_MS", max_batch_wait_ms)
    )

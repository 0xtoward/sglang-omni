...
def test_singleton_wait_policy_is_opt_in() -> None:
...
def test_queued_peer_restores_batch_wait_window() -> None:
    scheduler = _TestStreamingScheduler(max_batch_wait_ms=30)
    scheduler.inbox = _RecordingQueue()
    scheduler._wait_for_batch_peer = lambda msg: False
    first = IncomingMessage("a", "new_request", _payload("a"))
...
def test_no_wait_policy_preserves_control_stream_and_cost_boundaries() -> None:
...
def test_aborted_peer_does_not_start_wait_window() -> None:
    scheduler = _TestStreamingScheduler(max_batch_wait_ms=30)
    scheduler.inbox = _RecordingQueue()
    scheduler._wait_for_batch_peer = lambda msg: False
    scheduler.abort("b")
